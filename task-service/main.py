...
# 4. FastAPI Setup & Database Session Dependency
...
def publish_event(event_type: str, data: dict):
    try:
        ...
        # Declare a fanout exchange named 'task_events'
...
def update_task(task_id: int, task_data: TaskUpdate, db: Session = Depends(get_db)):
    ...
    # Publish TASK_UPDATED event to RabbitMQ
    event_payload = {
        "id": db_task.id,
        "title": db_task.title,
        "description": db_task.description,
...
def delete_task(task_id: int, db: Session = Depends(get_db)):
    ...
    if not db_task:
      ...
      # Store task metadata before deleting
    event_payload = {
        "id": db_task.id,
        "title": db_task.title
    }
    ...
    db.commit()
     ...
     # Publish TASK_DELETED event to RabbitMQ
